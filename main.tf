...
#role for the entire EKS cluster
...
#define the network security policy for the cluster, security group, attached to subnets in a VPC
# unrestricted outbound traffic, no inbound traffic allowed (no ingress defined)
...
#definition of the cluster itself
...
# node policy attached to the role
